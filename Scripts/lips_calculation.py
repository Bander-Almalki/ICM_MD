...
for filename in os.listdir(folder_path):
    if filename.endswith(".aln"):
        ...
        output_file=output_dir+sub_dir+filename.split(".")[0]+".lips"
        LIPS_score_calculation(msa_path,output_file)
        print(f"{filename} processed")

        print("parse LIPS score")
        LIPS_parsed_csv=output_dir+filename.split(".")[0]+".csv"
        parse_LIPS_score(output_file,LIPS_parsed_csv)
        print(f"{filename} processed")



# output_dir="LIPS_output/"
# if not os.path.exists(output_dir):
#     os.makedirs(output_dir)

# directory="dimer_MSAs_aln/"
# for filename in os.listdir(directory):
#     if filename.endswith(".a2m.aln"):
#         msa_path = os.path.join(directory, filename)
#         output_file=output_dir+filename.split(".")[0]+".lips"
#         LIPS_score_calculation(msa_path,output_file)
#     break
